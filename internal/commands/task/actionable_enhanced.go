...
// EnhancedActionableAction finds the next actionable task using dependency-aware selection
func EnhancedActionableAction(appCtx *shared.AppContext) cli.ActionFunc {
...
Examples:
  ...
  knot task actionable --verbose --json         # Detailed JSON output`,
		Action: EnhancedActionableAction(appCtx),
		Flags: []cli.Flag{
			&cli.StringFlag{
				Name:    "strategy",
				Aliases: []string{"s"},
