...
type Service interface {
	...
	InitializeFromCLIContext(c *cli.Context)

	// Utility methods
	ValidateConfig(c *ManagerConfig) error
...
func NewService(injector do.Injector) (Service, error) {
	return &serviceImpl{
		...
		isMCPMode:    false,
		managerConfig: DefaultConfig(), // Initialize with defaults
	}, nil
}
...
func (s *serviceImpl) GetMCPConfig() *MCPConfig {
	...
	return &MCPConfig{
		Address:  "localhost", // Default values
		Port:     8080,
		LogLevel: s.logLevel,
		Database: DatabaseConfig{
			Backend:   "postgres",
			Endpoint:  "", // Will be set from CLI context
		},
...
func (s *serviceImpl) ValidateConfig(c *ManagerConfig) error {
...
func (s *serviceImpl) InitializeFromCLIContext(c *cli.Context) {
	// Detect MCP mode
	...
	// Cache the manager configuration
	s.SetManagerConfig(managerConfig)
}

// Reset resets the configuration service to defaults
// Useful for testing
func (s *serviceImpl) Reset() {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.logLevel = "info"
	s.dbPath = ""
	s.isMCPMode = false
	s.managerConfig = DefaultConfig()
}