...
func (_q *SessionQuery) loadProject(ctx context.Context, query *ProjectQuery, nodes []*Session, init func(*Session), assign func(*Session, *Project)) error {
	...
	for i := range nodes {
		if nodes[i].session_project == nil {
			continue
		}
		fk := *nodes[i].session_project
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "session_project" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
